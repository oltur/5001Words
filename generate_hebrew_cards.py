#!/usr/bin/env python3
"""
Generates hebrew_cards.json (5,001 Hebrew-English flashcards).
Translates Spanish fronts from spanish_cards.json into Hebrew.
Front: Hebrew  Back: English. No audio.
Translating ES→HE avoids comma-separated multi-value English translations.
Usage: python3 generate_hebrew_cards.py
...
translator = GoogleTranslator(source="es", target="iw")
...
print(f"Translating {len(needed_unique)} unique Spanish strings ES→HE...")

for i in range(0, len(needed_unique), BATCH):
    batch = needed_unique[i:i+BATCH]
    try:
        translated = translate_batch(batch)
        for orig, he in zip(batch, translated):
            cache[orig] = he
        if (i // BATCH) % 10 == 0:
...
for card, spanish in zip(src, all_spanish):
    he_front = cache.get(spanish, spanish)
    result.append({"front": he_front, "back": card["back"]})
