#!/usr/bin/env python3
"""
Generates French TTS audio using edge-tts (Microsoft neural voices).
Writes MP3s to Audio.bundle/french/ in both iOS and Watch targets.
Usage: python3 generate_french_audio_edge.py [start] [end]
...
JSON_PATH = BASE / "french_cards.json"
OUT_DIR   = BASE / "Audio.bundle/french"
WATCH_OUT = BASE.parent / "YetAnotherLearningCards Watch App/Audio.bundle/french"
...
async def generate(audio_index: int, text: str):
    mp3_path = OUT_DIR / f"french_{audio_index}.mp3"
    if mp3_path.exists():
        return False
    comm = edge_tts.Communicate(text, VOICE)
    await comm.save(str(mp3_path))
    watch_path = WATCH_OUT / f"french_{audio_index}.mp3"
