#!/usr/bin/env python3
"""
Generates Dutch TTS audio using edge-tts (Microsoft neural voices).
Writes MP3s to Audio.bundle/dutch/ in both iOS and Watch targets.
Usage: python3 generate_dutch_audio_edge.py [start] [end]
...
async def generate(audio_index: int, text: str):
    mp3_path = OUT_DIR / f"dutch_{audio_index}.mp3"
    if mp3_path.exists():
        return False
    comm = edge_tts.Communicate(text, VOICE)
    await comm.save(str(mp3_path))
    watch_path = WATCH_OUT / f"dutch_{audio_index}.mp3"
