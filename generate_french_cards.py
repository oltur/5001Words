#!/usr/bin/env python3
"""
Generates french_cards.json (5,001 French-English flashcards).
Translates Spanish fronts → French, keeps English backs.
Usage: python3 generate_french_cards.py
...
OUT_JSON = BASE / "french_cards.json"
CACHE    = BASE / "french_cache.json"
...
print(f"Translating {len(needed_unique)} unique Spanish strings ES→FR...")

for i in range(0, len(needed_unique), BATCH):
    batch = needed_unique[i:i+BATCH]
    try:
        translated = translate_batch(batch)
        for orig, fr in zip(batch, translated):
            cache[orig] = fr
        if (i // BATCH) % 10 == 0:
...
result = []
for i, (card, spanish) in enumerate(zip(src, all_spanish)):
    fr_front = cache.get(spanish, spanish)
    result.append({"front": fr_front, "back": card["back"], "audioIndex": i})
