#!/usr/bin/env python3
"""
Generates Ukrainian TTS audio using edge-tts (Microsoft neural voices).
Writes MP3s to Audio.bundle/ukrainian/ in both iOS and Watch targets.
Usage: python3 generate_ukrainian_audio_edge.py [start] [end]
...
JSON_PATH = BASE / "ukrainian_cards.json"
OUT_DIR   = BASE / "Audio.bundle/ukrainian"
WATCH_OUT = BASE.parent / "YetAnotherLearningCards Watch App/Audio.bundle/ukrainian"
...
async def generate(audio_index: int, text: str):
    mp3_path = OUT_DIR / f"ukrainian_{audio_index}.mp3"
    if mp3_path.exists():
        return False
    comm = edge_tts.Communicate(text, VOICE)
    await comm.save(str(mp3_path))
    watch_path = WATCH_OUT / f"ukrainian_{audio_index}.mp3"
